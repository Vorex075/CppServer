...
bool Room::BroadcastMsg(const std::string& member_name, const std::string& buffer) const {
  if (this->partitipants_.find(member_name) != this->partitipants_.end()) {
    return -1;
  }
  bool sent_data{false};
  for (const auto& element: this->partitipants_) {
    if (socket_utils::SendData(element.second.get(), buffer.c_str(), buffer.size()) > 0) {
      sent_data = true;
    };
  }
  return true;
