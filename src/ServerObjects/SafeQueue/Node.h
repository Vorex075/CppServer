...
class QueueNode {
  public:
    ...
    QueueNode(const QueueNode<T>& another_node); // Copu constructor

    
    T& ReadValue() const { return this->value; }

    T GetValue() const { return std::move(this->value); }

    bool SetNext(std::unique_ptr<QueueNode<T>> next_node) { this->next_ = std::move(next_node); }
...
template<class T>
QueueNode<T>::QueueNode(QueueNode<T>&& another_node) {
...
template<class T>
QueueNode<T>::QueueNode(const QueueNode<T>& another_node) {
