...
bool Server::Run(int n_conections) {
  ...
  rooms = std::make_shared<std::map<std::string, socket_utils::Room>>();
  while (true) {
    auto peer_sock_ptr =
...
void HandleNewClient(
    std::shared_ptr<socket_utils::PeerSocketTCP> client,
    std::shared_ptr<std::map<std::string, socket_utils::Room>> rooms) {
  ...
  case server::ServerOptions::CREATE_ROOM:
    JoinRoom(client, std::move(name), CreateRoom(client.get(), rooms), rooms);
    break;
